...
function solve(filename) {
	...
	data.forEach((y, index) => {
		y.forEach((x, index2) => {
			if (x === 83 || x === 97) all_starts.push({ x: index2, y: index });
			if (x === 69) end = { x: index2, y: index };
		});
	});
	let smallest_till_now = Number.MAX_SAFE_INTEGER;
	let solutions = all_starts.map((start) => {
		...
		while (true) {
			...
			if (queue.length === 0 || queue[0].length > smallest_till_now) break;
		}
		if (solution.length) smallest_till_now = solution[0].length - 1;
		return solution.length ? solution[0].length - 1 : Number.MAX_SAFE_INTEGER;
	});
