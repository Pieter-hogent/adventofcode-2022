...
function solve(filename) {
	...
	let posHead = { x: 0, y: 0 };
	let posTail = Array.from({ length: 9 }, _ => ({ x: 0, y: 0 }));
	data.forEach(([dir, len]) => {
		for (let i = 0; i < len; i++) {
			posHead = move_head(posHead, dir);
			for (let j = 0; j < posTail.length; j++) {
				posTail[j] = (j === 0) ? move_tail(posTail[j], posHead) : move_tail(posTail[j], posTail[j - 1]);
			}

			all_visited.add(`${posTail.at(-1).x},${posTail.at(-1).y}`);
		}
	});
